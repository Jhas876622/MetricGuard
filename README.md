...
3. [Solution Overview](#3-solution-overview)
   - 3.5 [Dashboard — Live View](#35-dashboard--live-view)
...
## 3.5 Dashboard — Live View

**Overview:** KPI strip + top-ranked conflict (trust risk 100/100, MAU/active_users/churn cluster spanning 4 teams).

![MetricGuard dashboard overview](docs/screenshots/dashboard-overview.png)

**Agent recommendation:** per-team SQL breakdown + RAG-grounded canonical definition, generated by the four-step agent in §5.7.

![MetricGuard agent recommendation panel](docs/screenshots/dashboard-agent-recommendation.jpg)
...
│   ├── results.json    # Full machine-readable payload (generated, gitignored)
│   └── data.js          # Dashboard data entry-point (generated, gitignored)
│
├── tests/
...
> ⚠️ **Before opening the dashboard:** `output/results.json` and `output/data.js` are **gitignored** (they're generated, not committed). If you clone this repo fresh, `output/dashboard.html` will render blank until you run `python src/report.py` at least once. See [§9](#9-installation--usage).
...
| **0.65** | **4** (neural) | **0** (neural) | Correct on this dataset with neural embeddings |
| 0.72 | 3 | 0 | Churn pair (sim=0.67) missed |

With the neural model (all-MiniLM-L6-v2), all intended pairs score ≥ 0.70, making 0.65 conservatively safe.

**Verified edge case — TF-IDF fallback path (found and fixed):** When the neural model can't be downloaded (no network access — e.g. an offline CI runner or a restricted sandbox), the engine falls back to TF-IDF as designed. Testing this fallback path end-to-end surfaced a real threshold miss: the `conversion_rate` pair (Sales vs Marketing) scored **0.636** — just under the 0.65 threshold — so that conflict was silently *not* flagged, while churn (`0.670`) barely cleared it.

**Fix:** added `merge_exact_name_duplicates()` (`engine.py`) — a safety net layered on top of `find_semantic_groups()`, not inside it. Two metrics sharing the exact same `metric_name` are the same concept *by definition*, independent of embedding quality, so they're force-merged after the similarity-based grouping runs. Kept as a separate function (rather than folded into the union-find loop) so `find_semantic_groups()` stays pure and its existing unit tests didn't need to change. Verified: TF-IDF fallback now correctly finds all 5 groups (12/12 metrics, 100% conflict) instead of missing `conversion_rate`; all 34 tests still pass. This is a strong interview talking point: *"I found a real threshold edge case in my own fallback path, root-caused it to lexical vs semantic similarity, and fixed it with a deterministic override rather than just tuning the threshold blindly."*
...
**Worked examples from the dataset (neural embedding run):**
...
*(On the TF-IDF fallback path, churn separates into its own 2-team group at risk 70, and conversion_rate is missed entirely per the §5.3 edge case — same formula, different grouping input.)*
...
Running `python src/report.py` on the demo dataset (neural embedding path, `all-MiniLM-L6-v2`) produces:
...
> **Reproducibility note:** these figures assume `all-MiniLM-L6-v2` downloads successfully on first run. On a machine/environment with no internet access, the engine falls back to TF-IDF and produces slightly different numbers — **10/12 (83.3%) in conflict, 4 groups, peak risk 85**, with the `conversion_rate` pair missed (similarity 0.636, below the 0.65 threshold) and `churn` separating into its own 2-team group instead of merging with MAU. Verified by running the exact committed code end-to-end. Re-run `python src/report.py` yourself before an interview/demo to confirm which path your machine takes — `pip list | grep sentence-transformers` and check the console log for "falling back to TF-IDF" to know which numbers are live in your dashboard.
...
│
├── src/
│   ├── engine.py                  # 472 lines — core detection pipeline
...
│   │
│   ├── genai.py                   # 249 lines — RAG + LLM layer
...
│   │
│   └── report.py                  # 124 lines — pipeline orchestrator
...
│
├── output/
│   ├── dashboard.html             # Static interactive dashboard (committed)
...
`dashboard.html` is a static file opened directly from the filesystem (no web server). Browsers block `fetch()` requests to local files (`file://` protocol) due to CORS policy. Loading `data.js` as a `<script>` tag works unconditionally — it executes synchronously and sets `window.MG_DATA` before any rendering code runs. Trade-off: this is why `data.js` must be generated locally rather than fetched, and why it can't simply be committed as a static artifact without going stale.
...
**TF-IDF fallback threshold gap** — the `conversion_rate` pair scored 0.636 under the TF-IDF fallback (below the 0.65 threshold) and was silently missed. Fixed with `merge_exact_name_duplicates()` — see [§5.3](#53-semantic-grouping--union-find-algorithm) for the full root-cause and fix writeup.
...
**Generated dashboard data isn't committed:** `output/data.js` is gitignored by design (it's a build artifact), but this means the dashboard is non-functional on a fresh clone until `report.py` runs once. Worth adding a one-line note in a "Quick Start" badge or committing a static demo snapshot (`data.demo.js`) purely for recruiters who just want to see it render.
...
| **Debugging & fixing your own edge cases** | `engine.py: merge_exact_name_duplicates()` | "I found a real threshold miss in my TF-IDF fallback — conversion_rate scored 0.636, just under 0.65. Instead of just lowering the threshold (risking false positives elsewhere), I added a deterministic exact-name-match safety net as a separate function, so the core similarity logic stays pure and testable." |
...
Artificial Intelligence • Machine Learning • Data Engineering • Full Stack Development • Generative AI • Software Engineering

<p>

<a href="https://github.com/Jhas876622">
<img src="https://img.shields.io/badge/GitHub-Jhas876622-181717?style=for-the-badge&logo=github">
</a>

<a href="https://www.linkedin.com/in/satyam-kumar-jha-27545a288">
<img src="https://img.shields.io/badge/LinkedIn-Satyam%20Kumar%20Jha-0077B5?style=for-the-badge&logo=linkedin">
</a>

<a href="mailto:jha876622@gmail.com">
...
MetricGuard was designed as an end-to-end AI system that combines semantic search, SQL static analysis, Retrieval-Augmented Generation (RAG), and explainable AI to improve trust in enterprise analytics.
...
Their contributions continue to accelerate innovation in Artificial Intelligence and Data Engineering.
...
You are free to use, modify, distribute, and build upon this project, provided that the original copyright notice and license are retained.

See the [LICENSE](LICENSE) file for complete details. :contentReference[oaicite:1]{index=1}

---

<div align="center">

**Made with ❤️ by Satyam Kumar Jha**

*"Building trustworthy AI systems for the next generation of data-driven organizations."*
